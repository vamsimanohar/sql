...
/**
 * Integration tests for wildcard pattern matching with = and != operators in SQL. Tests behavior
 * with and without Calcite engine and pushdown optimization.
 */
public class WildcardEqualIntegrationIT extends SQLIntegTestCase {
  ...
  public void test_sql_wildcard_complex_pattern() throws IOException {
    ...
    JSONObject result = executeJdbcRequest(query);
    verifyDataRows(result, rows("test wildcard"), rows("test wildcard in body"));
  }
  ...
  public void test_sql_ip_field_wildcard() throws IOException {
    ...
    try {
      ...
      assertCondition(
          "IP field wildcard should require CAST",
          result.has("error") || result.getInt("total") == 0);
    } finally {
      disablePushdown();
      disableCalcite();

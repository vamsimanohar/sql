...
/**
 * Smart not-equals operator that automatically handles:
 * 1. Standard inequality for same-type comparisons
 * 2. Wildcard pattern matching negation when * or ? are detected in strings
 ...
 * 
 * Examples:
 * - name!='John' -> exact not match
 * - name!='J*' -> NOT wildcard match (names not starting with J)
 * - age!='3*' -> convert age to string, then NOT wildcard match
 * - '192.168.*'!=ip_field -> reverse NOT wildcard match on IP field
 */
public class SmartNotEquals implements FunctionImp2 {
  ...
  public RexNode resolve(RexBuilder builder, RexNode arg1, RexNode arg2) {
    ...
    if (wildcardPattern != null) {
      ...
      // Use NOT ILIKE for case-insensitive wildcard NOT matching
      return builder.makeCall(
          SqlStdOperatorTable.NOT,
          builder.makeCall(SqlLibraryOperators.ILIKE, field, patternNode));
    }
    ...
    if (isMixedTypeComparison(arg1, arg2)) {
      ...
      // This allows comparisons like age!='32' (exact not match)
      return handleMixedTypeNotEquals(builder, arg1, arg2);
    }
  ...
  /**
   * Handle mixed-type not-equals without wildcards (e.g., age!='32')
   */
  private RexNode handleMixedTypeNotEquals(RexBuilder builder, RexNode arg1, RexNode arg2) {
    ...
    if (arg1IsString) {
        ...
        try {
          if (arg1.isA(SqlKind.LITERAL)) {
            ...
            if (strValue != null && isNumeric(strValue)) {
              ...
              return builder.makeCall(SqlStdOperatorTable.NOT_EQUALS, castedArg1, arg2);
            }
          }
        } catch (Exception e) {
      ...
      return builder.makeCall(SqlStdOperatorTable.NOT_EQUALS, arg1, castedArg2);
    } else {

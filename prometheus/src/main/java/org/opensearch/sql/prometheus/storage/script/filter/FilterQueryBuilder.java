...
import org.opensearch.sql.prometheus.storage.script.filter.lucene.LuceneQuery;
import org.opensearch.sql.prometheus.storage.script.filter.lucene.RangeQuery;
import org.opensearch.sql.prometheus.storage.script.filter.lucene.RangeQuery.Comparison;
import org.opensearch.sql.prometheus.storage.script.filter.lucene.TermQuery;
import org.opensearch.sql.prometheus.storage.script.filter.lucene.WildcardQuery;
import org.opensearch.sql.prometheus.storage.script.filter.lucene.relevance.MatchQuery;
...
public class FilterQueryBuilder extends ExpressionNodeVisitor<QueryBuilder, Object> {
  ...
  private ScriptQueryBuilder buildScriptQuery(FunctionExpression node) {
    return new ScriptQueryBuilder(new Script(
        DEFAULT_SCRIPT_TYPE, ExpressionScriptEngine.EXPRESSION_LANG_NAME, serializer.serialize(node), emptyMap()));
  }

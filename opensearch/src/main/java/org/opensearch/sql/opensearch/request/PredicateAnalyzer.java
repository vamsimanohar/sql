...
public class PredicateAnalyzer {
  ...
  static class SimpleQueryExpression extends QueryExpression {
          ...
          if (isTextOrKeywordField() || !wildcardFieldRef.equals(getFieldReferenceForTermQuery())) {
            ...
            String script =
                String.format(
                    ...
                    getFieldReference(), getFieldReference(), convertWildcardToRegex(stringValue));
    ...
    /**
     * Check if the field is a keyword field that supports wildcard queries. IMPORTANT: OpenSearch
     * wildcard queries ONLY work on keyword fields, NOT text fields. Text, IP, numeric, date, and
     * other field types don't support wildcard queries.
     */
    ...
    /**
     * Get the appropriate field reference for wildcard queries. For text fields, automatically use
     * .keyword subfield if it exists.
     */
    private String getFieldReferenceForWildcard() {
      ...
      if (rel != null && rel.getExprType() != null && !fieldRef.endsWith(".keyword")) {
        ...
        String keywordField =
            OpenSearchTextType.toKeywordSubField(getFieldReference(), rel.getExprType());
    ...
    /**
     * Convert OpenSearch wildcard pattern to Java regex pattern. * -> .* (zero or more characters)
     * ? -> . (exactly one character) Other characters are escaped for regex.
     */

...
import org.opensearch.sql.planner.distributed.PartitionDiscovery;
...
public class OpenSearchPartitionDiscovery implements PartitionDiscovery {
  ...
  public List<DataPartition> discoverPartitions(String tableName) {
    ...
    try {
      ...
      // Handle comma-separated index patterns (e.g., "test,test1" or "bank,test*")
      ...
      // Find matching indexes for each pattern
      for (IndexRoutingTable indexRoutingTable : routingTable) {
        String indexName = indexRoutingTable.getIndex().getName();

        for (String pattern : patterns) {
          String trimmedPattern = pattern.trim();
          if (matchesPattern(indexName, trimmedPattern)) {
            log.debug("Processing index: {} for pattern: {}", indexName, trimmedPattern);
            ...
            break; // Don't add same index twice if it matches multiple patterns
          }
        }
      }
